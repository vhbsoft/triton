...
//must loop twice for both low and high entropy  
  ...
  while(1)
  {
    ...
    probe_packet_length = 1100;
    unsigned long initial_experiment_run_time = 30;//180;
    unsigned long later_experiment_run_time = 60;//300;
    ...
    if(UDPTrainReceiver(send_buffer, &send_buffer_length, probe_packet_length, initial_experiment_run_time,  later_experiment_run_time)!= 0)
    {
      fprintf(stderr, "ERROR #%d: UDP Train Receiver Error", UDP_TRAIN_RECEIVER_FAILED);
      return UDP_TRAIN_RECEIVER_FAILED;                    
    }
